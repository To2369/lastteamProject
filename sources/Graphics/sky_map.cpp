...
sky_map::sky_map(ID3D11Device* device, const wchar_t* filename, bool generate_mips)
{
	...
	create_vs_from_cso(device, "shader\\sky_map_vs.cso", sky_map_vs.GetAddressOf(), NULL, NULL, 0);
	create_ps_from_cso(device, "shader\\sky_map_ps.cso", sky_map_ps.GetAddressOf());
	create_ps_from_cso(device, "shader\\sky_box_ps.cso", sky_box_ps.GetAddressOf());
